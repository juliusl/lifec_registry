...
use crate::{
    ...
    ImageManifest, OCI_ARTIFACTS_MANIFEST_MEDIA_TYPE, ORAS_ARTIFACTS_MANIFEST_MEDIA_TYPE, ArtifactManifest, ImageIndex,
};
...
impl Store {
    /// Read content,
    ///
    pub async fn read_content<T>(response: Response<Body>) -> Option<T>
    where
        T: for<'a> Deserialize<'a>,
    {
        ...
        match hyper::body::to_bytes(body).await {
            Ok(bytes) =>  {
                ...
                if let Some(content_digest) = docker_content_digest {
                ...
                if let Some(response) = response {
                        ...
                        .and_then(|h| h.to_str().ok())
                    {
                        ...
                        | Some(OCI_ARTIFACTS_MANIFEST_MEDIA_TYPE) => {
                            let manifest = Store::read_content::<ArtifactManifest>(response).await;
                            eprintln!("{:#?}", manifest);
                        }
                        Some(DOCKER_V1_MANIFEST)
                        | Some(DOCKER_V2_MANIFEST)
                        | Some(OCI_IMAGE_MANIFEST) => {
                            let manifest = Store::read_content::<ImageManifest>(response).await;
                            eprintln!("{:#?}", manifest);
                        }
