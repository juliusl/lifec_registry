        ...
        if let Some(route) = route.take() {
            match self.method {
                Some(Method::GET) => {
                    ...
                    route.get(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                Some(Method::HEAD) => {
                    event!(Level::DEBUG, "adding path HEAD {path}");
                    route.head(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                Some(Method::DELETE) => {
                    event!(Level::DEBUG, "adding path DELETE {path}");
                    route.delete(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                _ => {
            ...
            match self.method {
                Some(Method::GET) => {
                    event!(Level::DEBUG, "adding path GET {path}");
                    get(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                Some(Method::HEAD) => {
                    event!(Level::DEBUG, "adding path HEAD {path}");
                    head(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                Some(Method::DELETE) => {
                    event!(Level::DEBUG, "adding path DELETE {path}");
                    delete(resolve_api.data(self.clone()).data(self.host.clone()).data(self.context.clone()))
                }
                _ => {
                    panic!("Unsupported method, {:?}", self.method)
                }
